...
供台灣國小教室固定觸控螢幕使用的兩人潔牙記錄系統。學生完成潔牙後，只需點選自己的座號；教師可登入查看月報、設定每週使用日期、報表行政資料及特殊日期。
...
列印樣式會隱藏登入、導覽與操作按鈕，保留表格框線及黑白可辨識符號。

## 9. 驗收檢查

正式使用前，請依 `SPEC.md` 第 36 節驗證全部 16 個案例，特別包括：
...
- 若要更正既有潔牙紀錄，V1.0 只能由具備 Supabase 管理權限的人員在 Dashboard 處理。
