...
- 兩張大型觸控卡片，一次點擊完成潔牙。
- 頭像右上角的低干擾請假操作及二次確認。
- 教師管理頁可在二次確認後清除今日誤登的完成或請假紀錄，使狀態恢復為未完成；學生端無法取消。

若資料庫已在舊版 schema 建立，請在 Supabase SQL Editor 執行 `supabase/migrations/20260915_teacher_delete_today_records.sql`，新增教師清除紀錄所需的最小權限與 RLS policy。
